  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[#e2e1d7] pb-5">
        <div>
          <h1 className="text-2xl font-extrabold text-[#1a1a1e] font-outfit">SaaS Guest CRM</h1>
          <p className="text-xs text-[#7a7974] mt-1 font-semibold">Understand and optimize relationships with your visitors</p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 bg-white border border-[#e2e1d7] p-4 rounded-2xl shadow-sm">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {filteredGuests.map((guest) => {
          return (
            <div 
              key={guest.id} 
              className="bg-white border border-[#e2e1d7] rounded-2xl p-5 hover:shadow-md transition-all flex flex-col justify-between space-y-5"
            >
              
              {/* Top Section Profile Info */}
              <div className="flex justify-between items-start gap-4">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-full bg-[#1b4332]/10 text-[#1b4332] border border-[#e2e1d7] flex items-center justify-center font-extrabold text-sm shadow-inner shrink-0">
              ...
              <div className="flex flex-wrap gap-1">
                {guest.tags.map((tag, i) => (
                  <span key={i} className={`px-2 py-0.5 rounded-full text-[9px] font-extrabold uppercase border flex items-center gap-1 ${
                    ...
                    'bg-[#f4f3ed] border-[#e2e1d7] text-slate-700'
                  }`}>
              ...
              {/* Lifetime stats grid */}
              <div className="grid grid-cols-3 gap-2 bg-[#fcfbf9] border border-[#e2e1d7] rounded-xl p-3.5 text-center text-xs">
                <div>
                  <span className="block font-extrabold text-[#1a1a1e]">{guest.totalBookings}</span>
                  <span className="text-[9px] text-[#7a7974] uppercase tracking-wider font-semibold">Total Bookings</span>
                </div>
                <div>
                  <span className="block font-extrabold text-[#1a1a1e]">{currencySymbol}{guest.totalSpending.toLocaleString()}</span>
                  <span className="text-[9px] text-[#7a7974] uppercase tracking-wider font-semibold">Total Spent</span>
                </div>
                <div>
                  <span className="block font-extrabold text-[#1a1a1e]">{guest.lastVisit || 'N/A'}</span>
                  <span className="text-[9px] text-[#7a7974] uppercase tracking-wider font-semibold">Last Visit</span>
                </div>
              </div>
              ...
              <div className="space-y-1.5 text-xs">
                {guest.preferences && (
                  <p className="text-[#1a1a1e] leading-snug">
                    <span className="font-bold text-[#1b4332] block uppercase tracking-wider text-[9px]">Preferences</span>
                    {guest.preferences}
                  </p>
                )}
                {guest.notes && (
                  <p className="text-[#7a7974] leading-snug italic">
                    <span className="font-bold text-[#1b4332] not-italic block uppercase tracking-wider text-[9px]">Staff Notes</span>
                    "{guest.notes}"
                  </p>
                )}
              </div>
        ...
        {filteredGuests.length === 0 && (
          <div className="col-span-2 text-center py-20 bg-white border border-[#e2e1d7] border-dashed rounded-2xl text-[#7a7974]">
          ...
          <div className="w-full max-w-xl bg-white border border-[#e2e1d7] rounded-2xl shadow-2xl p-6 space-y-5">
            ...
            <form onSubmit={handleSubmit} className="space-y-4 text-xs font-semibold text-[#1a1a1e]">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold text-[#7a7974] mb-1">Phone Number</label>
                  <input
                    type="text"
                    value={phone}
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold text-[#7a7974] mb-1">Last Visit Date</label>
                  <input
                    type="date"
                    value={lastVisit}
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold text-[#7a7974] mb-1">Total Bookings Count</label>
                  <input
                    type="number"
                    min={0}
                    value={totalBookings}
                ...
                <div>
                  <label className="block text-[10px] uppercase font-bold text-[#7a7974] mb-1">Total Spending ({currencySymbol})</label>
                  <input
                    type="number"
                    min={0}
                    value={totalSpending}
              ...
              {/* Tag selector */}
              <div>
                <label className="block text-[10px] uppercase font-bold text-[#7a7974] mb-2">Category Badges</label>
                    ...
                    <button
                      ...
                      className={`px-3 py-1 rounded-full text-[10px] font-bold border transition-all cursor-pointer ${
                        tags.includes(tagVal) 
                          ...
                          : 'bg-[#f4f3ed] text-slate-600 border-[#e2e1d7] hover:bg-[#e2e1d7]/40'
                      }`}
                    >
                      {tagVal}
                    </button>
              ...
              <div>
                <label className="block text-[10px] uppercase font-bold text-[#7a7974] mb-1">Special Preferences</label>
                <input
                  type="text"
                  value={preferences}
              ...
              <div>
                <label className="block text-[10px] uppercase font-bold text-[#7a7974] mb-1">Staff Observation Notes</label>
                <textarea
                  rows={2}
                  value={notes}
