  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-[#e2e1d7] pb-5">
        <div>
          <h1 className="text-2xl font-extrabold text-[#1a1a1e] font-outfit">SaaS Analytics Panel</h1>
          <p className="text-xs text-[#7a7974] mt-1 font-semibold">Track hotel traffic, conversions, and revenue channels</p>
        </div>
          ...
          {(['7d', '30d', '90d', '1y'] as const).map(tf => (
            <button
              ...
              className={`px-4 py-1.5 rounded-lg text-xs font-bold uppercase transition-all cursor-pointer ${
                timeframe === tf ? 'bg-white text-[#1b4332] shadow-sm' : 'text-[#7a7974] hover:text-[#1a1a1e]'
              }`}
            >
        ...
        {[
          ...
          <div key={i} className="bg-white border border-[#e2e1d7] rounded-2xl p-5 shadow-xs flex justify-between items-center">
            <div className="space-y-1">
              ...
              <h3 className="text-xl font-extrabold text-[#1a1a1e] font-outfit">{m.val}</h3>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="bg-white border border-[#e2e1d7] rounded-2xl p-6 shadow-sm space-y-6">
          <div>
            <h3 className="text-sm font-bold text-[#1a1a1e] uppercase tracking-wide">Revenue Channels</h3>
            <span className="text-[10px] text-[#7a7974]">Direct incoming settlement totals</span>
          </div>

          <div className="w-full h-44 bg-[#fcfbf9] border border-dashed border-[#e2e1d7] rounded-xl relative flex items-end pt-2">
            <svg viewBox="0 0 500 150" className="w-full h-full text-[#1b4332]" preserveAspectRatio="none">
              <defs>
              ...
              <polyline fill="none" stroke="#1b4332" strokeWidth="2.5" points={drawAreaPoints(activeData.revenue)} />
              ...
              {activeData.revenue.map((val, i) => {
                ...
                const y = height - (val / maxVal) * (height - 30) - 15;
                return <circle key={i} cx={x} cy={y} r="3.5" fill="#1b4332" stroke="white" strokeWidth="1" />;
              })}
            </svg>
        ...
        <div className="bg-white border border-[#e2e1d7] rounded-2xl p-6 shadow-sm space-y-6">
          <div>
            <h3 className="text-sm font-bold text-[#1a1a1e] uppercase tracking-wide">Occupancy Levels</h3>
            <span className="text-[10px] text-[#7a7974]">Average occupancy percentages</span>
          </div>

          <div className="w-full h-44 bg-[#fcfbf9] border border-dashed border-[#e2e1d7] rounded-xl relative flex items-end justify-between px-6 pt-2 pb-6">
            {activeData.occupancy.map((occ, idx) => {
              return (
                <div key={idx} className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end group">
                  <span className="text-[8px] font-bold text-[#1b4332] opacity-0 group-hover:opacity-100 transition-opacity">{occ}%</span>
                  <div 
                    className="w-8 bg-[#1b4332]/85 hover:bg-[#1b4332] rounded-t-md transition-all duration-500" 
                    style={{ height: `${occ * 0.9}%` }}
                  />
                  <span className="text-[9px] font-bold text-[#7a7974] uppercase tracking-wider">{activeLabels[idx]}</span>
                </div>
              );
            })}
          </div>
        ...
        <div className="bg-white border border-[#e2e1d7] rounded-2xl p-6 shadow-sm space-y-6">
          <div>
            <h3 className="text-sm font-bold text-[#1a1a1e] uppercase tracking-wide">Traffic & Lead Conversions</h3>
            <span className="text-[10px] text-[#7a7974]">Website clicks compared to checkout rates</span>
          </div>
          ...
          <div className="flex gap-4 text-[10px] font-bold text-[#7a7974] justify-center pt-2">
        ...
        <div className="bg-white border border-[#e2e1d7] rounded-2xl p-6 shadow-sm space-y-5 flex flex-col justify-between">
          <div className="border-b border-[#e2e1d7] pb-3">
            <h3 className="text-sm font-bold text-[#1a1a1e] uppercase tracking-wide">AI Receptionist Efficiency</h3>
            <span className="text-[10px] text-[#7a7974]">AI chatbot booking ratios</span>
          </div>

          <div className="space-y-4 font-semibold text-xs text-[#1a1a1e]">
            <div className="p-3.5 bg-[#fcfbf9] border border-[#e2e1d7] rounded-xl flex justify-between items-center">
