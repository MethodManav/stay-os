  ...
  return (
    <div className="space-y-8">
      ...
      <div className="border-b border-border-subtle pb-5">
        <h1 className="text-2xl font-extrabold text-text-primary font-outfit">Payment Ledger</h1>
        <p className="text-xs text-text-secondary mt-1 font-semibold">Track transactions, refunds, and gateway settings</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white border border-border-subtle rounded-2xl p-6 shadow-sm space-y-4 flex flex-col justify-between">
          <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-center gap-3 border-b border-border-subtle pb-3">
            <div>
              <h3 className="text-sm font-bold text-text-primary uppercase tracking-wide">Transactions Ledger</h3>
              <span className="text-[10px] text-text-secondary">View transactional records from bookings</span>
            </div>
            ...
            <div className="flex gap-2 text-xs">
              <input
                ...
                className="px-3 py-1.5 border border-border-subtle rounded-xl focus:outline-none focus:border-brand-primary bg-[#f1f5f9]/30"
              />
              <select
                value={methodFilter}
                onChange={e => setMethodFilter(e.target.value)}
                className="px-2 py-1.5 border border-border-subtle rounded-xl focus:outline-none bg-[#f1f5f9]/30 font-bold"
              >
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs">
              <thead>
                <tr className="text-text-secondary border-b border-border-subtle uppercase tracking-wider font-bold">
              ...
              <tbody className="divide-y divide-border-subtle/60 text-text-primary font-semibold">
                {filteredBookings.map((b) => (
                  <tr key={b.id} className="hover:bg-slate-50">
                    ...
                    <td className="py-3 font-mono text-text-secondary">{b.id}</td>
                    <td className="py-3 font-extrabold text-sm">{currencySymbol}{b.amountPaid.toLocaleString()}</td>
                    <td className="py-3 text-text-secondary">{b.paymentMethod}</td>
                    <td className="py-3">
                ...
                {filteredBookings.length === 0 && (
                  <tr>
                    <td colSpan={5} className="text-center py-6 text-text-secondary italic">
        ...
        <div className="bg-white border border-border-subtle rounded-2xl p-6 shadow-sm space-y-6">
          <div className="border-b border-border-subtle pb-3">
            <h3 className="text-sm font-bold text-text-primary uppercase tracking-wide">Gateway Integration</h3>
          ...
          <div className="space-y-4 text-sm font-semibold text-text-primary">
            ...
            <div className="p-3.5 border border-border-subtle bg-bg-page rounded-xl space-y-2">
              <div className="flex justify-between items-center">
                <span className="font-extrabold text-text-primary">Razorpay Sandbox</span>
                <span className="px-2 py-0.5 bg-emerald-100 text-emerald-800 rounded font-bold text-[11px]">Active</span>
              </div>
              <input
                type="text"
                disabled
                value="rzp_test_5N89aP12b"
                className="w-full p-2 bg-[#f1f5f9]/60 border border-border-subtle rounded text-slate-500 font-mono"
              />
            </div>
            ...
            <div className="p-3.5 border border-border-subtle bg-bg-page rounded-xl space-y-2">
              <div className="flex justify-between items-center">
                <span className="font-extrabold text-text-primary">Stripe Integration</span>
                <span className="px-2 py-0.5 bg-slate-100 text-slate-600 rounded font-bold text-[11px]">Inactive</span>
              </div>
              <button className="w-full py-2 bg-brand-primary hover:bg-brand-hover text-white font-bold text-xs rounded transition-colors">
